...
local X_RATELIMIT_LIMIT_KEY = "X-Ratelimit-Limit-Key"
local X_RateLimit_KEY_Remaining = "X-RateLimit-Key-Remaining"

local LimitKeyQuotaHandler = {
...
local ERR_INVALID_LIMIT_KEY_QUOTA = { status = 429, message = "API rate limit exceeded" }

local function increment(key, value, exp)
  local newval, err = shm:incr(key, value, 0, exp)
  kong.log.err(newval)
  if not newval then
    kong.log.err("could not increment counter for exp '", exp, "': ", err)
    return nil, err
  end
  return true
end
...
local function usage(key)
  ...
  local current_metric, err = shm:get(key)
  if err then
    return nil, err
  end
...
local function load_credential(key)
  local cred, err = kong.db.limit_key_quota_credentials:select_by_key(key)
  if not cred then
    return nil, err
  end
...
local function do_authentication(conf)
  ...
  local credential_cache_key = kong.db.limit_key_quota_credentials:cache_key(key)
  ...
  if not credential or hit_level == 4 then
    ...
    local limit = credential.limit_key_quota
    kong.response.set_header(X_RATELIMIT_LIMIT_KEY, limit)

    local increment, err = increment(key, 1, credential.ttl)
    local usage, err = usage(key)
    if err then
    ...
    if remaining < 0 then
      ERR_INVALID_LIMIT_KEY_QUOTA.status = conf.error_code
      ERR_INVALID_LIMIT_KEY_QUOTA.message = conf.error_message
      kong.response.set_header(X_RateLimit_KEY_Remaining, 0)
      return nil, ERR_INVALID_LIMIT_KEY_QUOTA
    end
    ...
    kong.response.set_header(X_RateLimit_KEY_Remaining, remaining)
...
function LimitKeyQuotaHandler:access(conf)
